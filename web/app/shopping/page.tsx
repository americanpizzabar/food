    ...
    <div className="space-y-5 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">残り {uncheckedCount}品 / 合計 {items.length}品</p>
        </div>
      ...
      {showDeleteAll && (
        <div className="card bg-red-50 border-red-200">
          <p className="text-sm text-red-700 mb-3">買い物リストを全て削除しますか？</p>
      ...
      {showForm && (
        <div className="card space-y-3 animate-fade-in">
          <div className="flex justify-between items-center">
            <h2 className="font-bold text-gray-800">アイテムを追加</h2>
          ...
          <div className="flex flex-wrap gap-2">
            {CATEGORIES.map(c => (
              <button key={c} onClick={() => setForm(f => ({ ...f, category: c }))}
                className={`px-3 py-1 rounded-full text-sm border transition-all ${form.category === c ? 'bg-blue-500 text-white border-blue-500' : 'bg-white border-gray-200'}`}>
                {c}
              </button>
            ))}
          </div>
        ...
        {Object.entries(grouped).map(([category, catItems]) => (
          <div key={category}>
            <div className="bg-gray-100 rounded-lg px-3 py-1.5 mb-2">
              <span className="text-xs font-semibold text-gray-600">{category === 'その他2' ? 'その他' : category} ({catItems.length})</span>
            </div>
            <div className="space-y-1">
              {catItems.map(item => (
                <div key={item.id}
                  className={`flex items-center gap-3 p-3 rounded-xl border transition-all ${item.isChecked ? 'bg-gray-50 border-gray-100 opacity-60' : 'bg-white border-gray-100'}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm font-medium ${item.isChecked ? 'line-through text-gray-400' : 'text-gray-800'}`}>{item.name}</p>
